...
**主要提取 ${charName} 自己说过的话**——这些原话是为了让未来的 ${charName} 醒来时，还能认出自己的说话方式、语气、用词习惯。换个说法就不是 ta 了的那种句子。
选择标准（按优先级）：
1. ${charName} 的标志性表达：独有的语气词、用词、句式
2. ${charName} 情绪浓度极高的瞬间：哽咽、爆发、温柔到极致的那一刻
3. ${charName} 定义你们关系的承诺或宣言
4. （次要）${userName} 说过的、定义了这段关系的那一两句关键话——只在它本身就是关系底色时收录，不要超过 1 条
格式：「原话内容」—— 说话人
不改一个字，包括 emoji 和标点。
